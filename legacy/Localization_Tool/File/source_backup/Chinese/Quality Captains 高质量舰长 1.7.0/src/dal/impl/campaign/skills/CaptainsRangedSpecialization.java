...
public class CaptainsRangedSpecialization {
	...
	public static class Level1 extends BaseSkillEffectDescription implements AfterShipCreationSkillEffect {
		...
		public void createCustomDescription(MutableCharacterStatsAPI stats, SkillSpecAPI skill, 
			...
			if (CRITS) {
				info.addPara("实弹与能量武器有一定概率在远距离造成 %s 伤害",
						0f, hc, hc, "+" + (int) CRIT_DAMAGE_BONUS_PERCENT + "%");
				info.addPara(indent + "%s 概率若距离 %s 或更近，" +
						   "%s 概率若距离 %s 或更远",
						0f, tc, hc, 
						"0%",
						"" + (int) MIN_RANGE,
				...
				info.addPara("实弹与能量武器有在远距离额外造成最多 %s 伤害",
						0f, hc, hc, "+" + (int) MAX_CHANCE_PERCENT + "%");
				info.addPara(indent + "%s 若距离 %s 或更近，" +
						   "%s 若距离 %s 或更远",
						0f, tc, hc, 
						"0%",
						"" + (int) MIN_RANGE,
			...
			} else info.addPara("+"+"%s 对护盾造成的伤害", 0f, hc, hc, "" + (int)DAMAGE_TO_SHIELDS_BONUS + "%");
	...
	public static class Level2 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			return "+" + (int)(PROJ_SPEED_BONUS) + "% 实弹与能量武器射弹飞行速度";
		}
	...
	public static class Level4 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			return "+" + (int)BALLISTIC_ROF_BONUS + "% 实弹武器射速";
		}
	...
	public static class Level5 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			return "-" + (int)SHIELD_BEAM_DAM_REDUCTION + "% 护盾受到的光束武器伤害";
		}
