	...
	public static float OVERLOAD_REDUCTION = 20;
		...
		public static class Level0 implements DescriptionSkillEffect {
			public String getString() {
				if (!loadStock) {
					return "\n\n*对于增益，所有舰船被视为处于至少 " + (int) MIN_BONUS_PERC + "% 硬幅能水平。对于无盾舰船，视为 " + (int) MIN_BONUS_SHIELDLESS_PERC + "%";
				} else {
		...
		public static class Level1 implements ShipSkillEffect {
			...
			public String getEffectDescription(float level) {
				return "+" + Math.round(MAX_DAMAGE_REDUCTION_BONUS * 100) + "% 装甲的最大伤害减免";
			}
		...
		public static class Level1B implements ShipSkillEffect {
			...
			public String getEffectDescription(float level) {
				return "+" + Math.round(MIN_ARMOR_FRACTION_BONUS * 100f) + "% 剥落装甲的减伤效果下限";
			}
		...
		public static class Level2 implements AfterShipCreationSkillEffect {
			...
			public String getEffectDescription(float level) {
				//return "Up to +" + (int)Math.round(ARMOR_FRACTION_BONUS * 100f) + "% of base 仅用于计算减伤的装甲值, based on current hard flux level";
				
				return "根据当前硬幅能* 水平，最多 +" + (int)(EFFECTIVE_ARMOR_BONUS) + "% 装甲值 (仅用于计算伤害减免*)";
			}
			public String getEffectPerLevelDescription() {
				return null;
			}
		...
		public static class Level3 implements ShipSkillEffect {
			...
			public String getEffectDescription(float level) {
				return "根据当前硬幅能* 水平，最多 -" + (int)Math.round(EMP_BONUS_PERCENT) + "% 所受到的 EMP 效果";
			}
			public String getEffectPerLevelDescription() {
				return null;
			}
		...
		public static class Level3B implements ShipSkillEffect {
			...
			public String getEffectDescription(float level) {
				return "-" + Math.round(OVERLOAD_REDUCTION) + "% 过载持续时间";
			}
		...
		public static class Level4 implements ShipSkillEffect {
			...
			public String getEffectDescription(float level) {
				return "-" + (int)(HULL_DAM_REDUCTION) + "% 船体结构受到的伤害";
