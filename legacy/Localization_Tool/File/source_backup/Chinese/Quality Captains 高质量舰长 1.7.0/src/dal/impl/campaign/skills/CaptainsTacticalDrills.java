...
public class CaptainsTacticalDrills {
	...
	public static class Level1 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
				return "-" + (int)(WEAPON_RECOIL_NEG_PERC) + "% 武器后坐力， +" + (int)(TURRET_LEADING_PERC) + "% 自动开火武器的精度";
			} else {
	...
	public static class Level2 extends BaseSkillEffectDescription implements FleetStatsSkillEffect {
		public void apply(MutableFleetStatsAPI stats, String id, float level) {
			if (!loadStock) {
				stats.getDynamic().getMod(Stats.PLANETARY_OPERATIONS_MOD).modifyPercent(id, GROUND_ATTACK_BONUS, "战术演习");
			} else {
		...
		public void createCustomDescription(MutableCharacterStatsAPI stats, SkillSpecAPI skill, TooltipMakerAPI info, float width) {
			if (!loadStock) {
				...
				info.addPara("影响: %s", opad + 5f, Misc.getGrayColor(), c, "地面作战");
				info.addSpacer(opad);
				info.addPara("+%s 突袭等地面作战行动的效率", 0f, hc, hc,
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
				return "+" + (int)(GROUND_ATTACK_BONUS) + "% 突袭等地面作战行动的效率， +" + (int)(GROUND_DEFENSE_BONUS) + "% 地面防御强度";
			} else {
	...
	public static class Level2B implements MarketSkillEffect {
		public void apply(MarketAPI market, String id, float level) {
			market.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).modifyMult(id, 1f + GROUND_DEFENSE_BONUS * 0.01f, "战术演习");
		}
		...
		public String getEffectDescription(float level) {
			return "+" + (int)(GROUND_DEFENSE_BONUS) + "% 额外地面防御强度";
		}
	...
	public static class Level3 implements FleetStatsSkillEffect {
		public void apply(MutableFleetStatsAPI stats, String id, float level) {
			if (!loadStock) {
				stats.getDynamic().getStat(Stats.PLANETARY_OPERATIONS_CASUALTIES_MULT).modifyMult(id, CASUALTIES_MULT, "战术演习");
			} else {
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
			return "-" + (int)Math.round((1f - CASUALTIES_MULT) * 100f) + "% 地面作战的伤亡";
			} else {
