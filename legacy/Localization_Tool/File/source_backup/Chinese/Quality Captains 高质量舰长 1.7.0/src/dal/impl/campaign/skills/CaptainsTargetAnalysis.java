...
public class CaptainsTargetAnalysis {
	...
	public static class Level1 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			...
			*/
			desc += "+" + Math.round(DAMAGE_TO_FRIGATES) + "/" + Math.round(DAMAGE_TO_DESTROYERS) + "/" + Math.round(DAMAGE_TO_CRUISERS) + "/" + Math.round(DAMAGE_TO_CAPITALS) + "% 额外伤害，根据目标的船体等级";
			return desc;
			} else {
	...
	public static class Level2 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
				return "+" + (int)(DAMAGE_TO_SHIELDS_BONUS) + "% 对护盾的伤害";
			} else {
	...
	public static class Level3 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
				return "+" + (int)(HIT_STRENGTH_BONUS) + "% 对装甲的有效打击强度";
			} else {
	...
	public static class Level4 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			if (!loadStock) {
				return "+" + (int)(DAMAGE_TO_COMPONENTS_BONUS) + "% 对武器和引擎的伤害";
			} else {
