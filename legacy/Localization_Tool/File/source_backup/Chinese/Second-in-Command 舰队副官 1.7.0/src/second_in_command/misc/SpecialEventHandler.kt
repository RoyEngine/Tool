    ...
    fun checkChristmas() {
        ...
        if (isChristmas) {
            if (data.getOfficersInFleet().none { it.aptitudeId == "sc_christmas" }) {
                ...
                Global.getSector().addScript(object : DelayedActionScript(0.5f) {
                    override fun doAction() {
                        Global.getSector().campaignUI.addMessage(" - 一位节日妖精加入了你的指挥团队", Misc.getHighlightColor())
                    }
                })
            }
            ...
            if (officer != null) {
                ...
                Global.getSector().addScript(object : DelayedActionScript(0.5f) {
                    override fun doAction() {
                        Global.getSector().campaignUI.addMessage(" - 节日妖精已经离开了你的舰队", Misc.getHighlightColor())
                    }
                })
