...
class BestOfTheBest : SCBaseSkillPlugin() {
    ...
    override fun addTooltip(data: SCData, tooltip: TooltipMakerAPI) {
        ...
        /*if (shipMasteryMode) {
            ...
            tooltip.addPara("+1 舰船可内置插件*数上限", 0f, Misc.getHighlightColor(), Misc.getHighlightColor())
            tooltip.addPara("   - 如果该执行军官被撤销指派，超出上限的内置船插将会失效", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "")
            tooltip.addPara("   - 重新指派该执行军官会使失效内插恢复生效", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "")

            tooltip.addSpacer(10f)

            tooltip.addPara("*舰船基础可内置插件的上限为 ${Misc.MAX_PERMA_MODS}}。" +
