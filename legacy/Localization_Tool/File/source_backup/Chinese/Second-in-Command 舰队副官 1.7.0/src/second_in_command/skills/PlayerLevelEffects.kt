...
object PlayerLevelEffects {
    ...
    fun getTooltipForLevel(level: Int) : String{
        ...
        if (level == 2) return "+10%% 执行军官获得的经验"
        if (level == 4) return "+10%% 执行军官获得的经验"
        if (level == 6) return "+10%% 执行军官获得的经验"
        if (level == 8) return "+20%% 执行军官获得的经验"
        if (level == 10) return "+2 你能指挥的军官人数上限"
        if (level == 12) return "+1 你能指挥的军官等级上限"
        if (level == 14) return "+1 你能指挥的军官精英技能数量上限"
        if (level == 15 && Global.getSettings().modManager.isModEnabled("nexerelin")) return "+1 你能指挥的特工数量上限"
