...
class WolfpackTactics : SCBaseSkillPlugin() {
    ...
    override fun addTooltip(data: SCData, tooltip: TooltipMakerAPI) {
        tooltip.addPara("护卫舰和驱逐舰对船体级别大于自身的舰船造成的伤害增加", 0f, Misc.getHighlightColor(), Misc.getHighlightColor())
        tooltip.addPara("   - +20%% 护卫舰对更大目标造成的伤害", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "+20%")
        tooltip.addPara("   - +10%% 驱逐舰对更大目标造成的伤害", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "+10%")

        tooltip.addSpacer(10f)

        tooltip.addPara("护卫舰和驱逐舰的性能峰值时间增加", 0f, Misc.getHighlightColor(), Misc.getHighlightColor())
        tooltip.addPara("   - +50%% 护卫舰峰值时间", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "+50%")
        tooltip.addPara("   - +25%% 驱逐舰峰值时间", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "+25%")
