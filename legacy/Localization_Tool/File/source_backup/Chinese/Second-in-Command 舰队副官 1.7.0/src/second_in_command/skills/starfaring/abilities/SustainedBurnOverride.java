...
public class SustainedBurnOverride extends SustainedBurnAbility {
    ...
    protected void applyEffect(float amount, float level) {
        ...
        //fleet.getStats().getSensorRangeMod().modifyMult(getModId(), 1f + (SENSOR_RANGE_MULT - 1f) * level, "持续加速");
        fleet.getStats().getDetectedRangeMod().modifyPercent(getModId(), DETECTABILITY_PERCENT * level, "持续加速");
...
//		}
        fleet.getStats().getFleetwideMaxBurnMod().modifyFlat(getModId(), burnModifier, "持续加速");
        fleet.getStats().getFleetwideMaxBurnMod().modifyMult(getModId(), burnMult, "持续加速");
        fleet.getStats().getFleetwideMaxBurnMod().modifyPercent(getModId(), MAX_BURN_PERCENT, "持续加速");
...
//			}
        ...
        if (fleet.getFleetData() != null) {
            ...
            if (data.isSkillActive("sc_starfaring_reactive_burn")) {
                ...
                data.getFleet().getStats().addTemporaryModMult(0.1f, "sc_reactive_burn", "响应驱动", 0f, data.getFleet().getStats().getFuelUseHyperMult());
                data.getFleet().getStats().addTemporaryModMult(0.1f, "sc_reactive_burn", "响应驱动", 0f, data.getFleet().getStats().getFuelUseNormalMult());
