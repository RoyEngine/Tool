...
object CodexHandler {
    ...
    fun addModEntry() {
        ...
        var modEntry = ModEntry(LunaCodex.getModEntryId("second_in_command"), "舰队副官", path)
...
class AptitudeEntry(var aptitudeSpec: SCAptitudeSpec, id: String, title: String, icon: String) : CodexEntryV2(id,  title, icon) {
    ...
    override fun createTitleForList(info: TooltipMakerAPI?, width: Float, mode: CodexEntryPlugin.ListMode?) {
        ...
        info.addPara("天赋", Misc.getGrayColor(), 0f)
    }
    ...
    override fun createCustomDetail(panel: CustomPanelAPI?, relatedEntries: UIPanelAPI?, codex: CodexDialogAPI?) {
        ...
        tooltip.addSectionHeading("描述", Alignment.MID, 0f)
        ...
        if (categories.isNotEmpty()) {
            ...
            tooltip.addSpacer(10f)
            tooltip.addPara("这一天赋属于 $categoryText 类别。因此它无法与其他属于同一类别的天赋同时生效。", 0f, Misc.getGrayColor(), aptitudePlugin.color, "$categoryText")
        }
...
class ModEntry(id: String, title: String, icon: String) : CodexEntryV2(id,  title, icon) {
    ...
    override fun createCustomDetail(panel: CustomPanelAPI?, relatedEntries: UIPanelAPI?, codex: CodexDialogAPI?) {
        ...
        tooltip.addPara("舰队副官 是一款重新构造了技能系统的模组。" +
                "在原版系统中，你可以在{战斗}、{领导}、{科技}和{工业}四大类能力中选择。",0f, Misc.getTextColor(), Misc.getHighlightColor(),
            "战斗", "领导", "科技", "工业")
        tooltip.addSpacer(10f)

        tooltip.addPara("在本模组中，玩家只保留了{战斗}技能。其他技能则被指派一种新的船员类别 -- \"执行军官\" 所取代。 " +
                "执行军官有着自己擅长的能力，当他们被分配到{三个可用名额}中的一个时，他们便能使自己的天赋{技能}发挥效用。",
            0f, Misc.getTextColor(), Misc.getHighlightColor(), "战斗", "执行军官", "三个可用名额", "技能")
        tooltip.addSpacer(10f)
        ...
        tooltip.addSpacer(10f)
        tooltip.addPara("执行军官的等级与玩家无关，最高等级为 5 级。默认情况下，他们已解锁一项技能，并可学习另外 5 项技能。只要你靠近殖民地，就可以随时自由更换指派的军官。",
            0f, Misc.getTextColor(), Misc.getHighlightColor(), "level independently", "5", "5")
        tooltip.addSpacer(10f)

        tooltip.addPara("他们通常会出现在殖民地的通讯名单中，也可能出现在打捞沉船时发现的冬眠舱之中。",
