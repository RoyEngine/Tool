...
public class VanillaSkillTooltip extends BaseTooltipCreator {
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
       ...
       /* if (skill.getScopeStr() != null) {
        ...
        tooltip.addPara("影响：{座舰}" + extra, 0f, Misc.getGrayColor(), Misc.getBasePlayerColor(), "座舰", "座舰部署的战机");
        tooltip.addSpacer(10f);
        ...
        for (HullModSpecAPI hullmodSpec: hullmodSpecs) {
            tooltip.addPara("船体插件：{" + hullmodSpec.getDisplayName() + " - " + ReflectionUtils.invoke("getShortDesc", hullmodSpec, new Array[]{}, null, null) , 0f,
                    Misc.getTextColor(), Misc.getHighlightColor(), hullmodSpec.getDisplayName());
        }

        if (skill.getId().equals("polarized_armor")) {
            tooltip.addSpacer(10f);
            tooltip.addPara("*没有护盾或相位系统的舰船被本技能视作永远处于 50%% 的硬幅能状态下。", 0f, Misc.getGrayColor(), Misc.getHighlightColor(), "50%");
        }

        if (requiredSkillPoints >= 2) {
            tooltip.addSpacer(10f);
            tooltip.addPara("需要 " + requiredSkillPoints + " 技能点。", 0f, Misc.getNegativeHighlightColor(), Misc.getNegativeHighlightColor());
        }
