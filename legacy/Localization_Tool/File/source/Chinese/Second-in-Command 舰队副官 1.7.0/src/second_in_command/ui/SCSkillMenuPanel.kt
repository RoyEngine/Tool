...
class SCSkillMenuPanel(var parent: UIPanelAPI,
    ...
    fun addHeaderPanel() {
        ...
        headerElement.addSectionHeading(if (isVanillaSectionEnabled) "" else "执行军官", Alignment.MID, 0f).apply {
            position.inTL(-10f, 0f)
            position.setSize(width-20, 20f)
        }

        if (isVanillaSectionEnabled) {
            var executiveButton = ClickableTextButton("执行军官", Misc.getBasePlayerColor(), headerElement, 200f, 20f)
            ...
            headerElement.addTooltip(executiveButton.elementPanel, TooltipMakerAPI.TooltipLocation.BELOW, 450f) { tooltip ->
                tooltip.addPara("执行军官提供能对整支舰队生效的技能。你能在殖民地的通讯目录雇佣、或是在休眠舱之中找到他们。" +
                        "")
            }


            var otherSkillsButton = ClickableTextButton("其他技能", Misc.getBasePlayerColor(), headerElement, 200f, 20f)
            ...
            headerElement.addTooltip(otherSkillsButton.elementPanel, TooltipMakerAPI.TooltipLocation.BELOW, 450f) { tooltip ->
                tooltip.addPara("此部分显示由模组添加、无法通过副官系统管理的原版技能。此处列出的技能无法通过技能点获取，仅供查看，但部分模组可能提供独立解锁途径。")
            }
    ...
    fun addAptitudePanel() {
        ...
        if (!title) {
            ...
            subelement.addPara("执行军官只能在战役中选择。", 0f, Misc.getTextColor(), Misc.getHighlightColor())
        }
    ...
    fun recreateAptitudeRow(subpanelParent: CustomPanelAPI, officer: SCOfficer?, slotId: Int) {
        ...
        if (!isLocked) {
            ...
            subelement.addTooltip(officerPickerElement.elementPanel, TooltipMakerAPI.TooltipLocation.BELOW, 350f) { tooltip ->
                tooltip.addPara("在玩家达到 $progressionLevel 级之前，此插槽将被锁定，解锁前无法使用。", 0f
...
*/
        ...
        var officerPara = subelement.addPara("${officer.person.nameString} - $spRemaining 技能点", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "$spRemaining")
        ...
        paraAnchorElement.advance {
            ...
            officerPara.text = "${officer.person.nameString} - $spRemaining 技能点"
    ...
    fun checkToApplyCRPenalty() {
        if (!isAtColony) {
            ...
            Global.getSector().campaignUI.messageDisplay.addMessage(
                "由于在殖民地范围之外调动执行军官，导致所有舰船承受 $cost% 战备值惩罚。",
    ...
    fun openResrictedOfficerManagementPanel(panel: CustomPanelAPI, subpanelParent: CustomPanelAPI, officer: SCOfficer) {
        ...
        element.addTooltip(portraitElement.elementPanel, TooltipMakerAPI.TooltipLocation.BELOW, 350f ) {
            it.addPara("点击以修改军官头像。你可以从任何开始新游戏时的 玩家可选头像 当中选择。", 0f)
        }
