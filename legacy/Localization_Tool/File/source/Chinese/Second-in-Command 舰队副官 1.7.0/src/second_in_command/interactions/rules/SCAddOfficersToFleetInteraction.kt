...
class SCAddOfficersToFleetInteraction : BaseCommandPlugin() {
    override fun execute(ruleId: String?, dialog: InteractionDialogAPI?, params: MutableList<Misc.Token>?, memoryMap: MutableMap<String, MemoryAPI>?): Boolean {
        ...
        if (isBoss) {
            dialog.textPanel.addPara("敌对{异常造物}似乎具备以下{技能}：", Misc.getTextColor(), Misc.getHighlightColor(), "异常造物", "技能")
        }
        else if (!automated) {
            dialog.textPanel.addPara("敌方舰队负责指挥的{执行军官}具备以下技能：", Misc.getTextColor(), Misc.getHighlightColor(), "执行军官")
        } else {
            dialog.textPanel.addPara("敌方舰队指令核心足以履行{执行军官}职责的子程序具备以下技能：", Misc.getTextColor(), Misc.getHighlightColor(), "执行军官")
        }
