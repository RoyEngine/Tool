...
public class SCCRPluginImpl implements CombatReadinessPlugin {
    ...
    public static float SERIOUS_MALFUNCTION_START = 0.3f;
...
//				//member.getFleetData().getFleet().getName().contains("Ryan") &&
//				 member.getCrewFraction() < 1f) {
//			System.out.println("efwefwefwefe");
//		}

        //float maxCRBasedOnLevel = (40f + member.getCrewFraction() * 10f) / 100f;
        float maxCRBasedOnLevel = 0.7f;
        member.getStats().getMaxCombatReadiness().modifyFlat("船员技能增益", maxCRBasedOnLevel, "基础维护");

        float cf = member.getCrewFraction();
        if (cf < 1) {
            float penalty = 0.5f * (1f - cf);
            //float penalty = (1f - cf);
            member.getStats().getMaxCombatReadiness().modifyFlat("船员不足", -penalty, "船员不足");
        } else {
            member.getStats().getMaxCombatReadiness().unmodifyFlat("船员不足");
        }
    }
    ...
    public List<CRStatusItemData> getCRStatusDataForShip(ShipAPI ship) {
        ...
        String title = "战备值 " + Math.round(cr * 100f) + "%";
        ...
        if (ship.losesCRDuringCombat() && cr > 0) {
            ...
            if (noLossTime > ship.getTimeDeployedForCRReduction()) {
                CRStatusItemData itemData = new CRStatusItemData(statusKeys[5], icon, "最佳运行性能",
                        "剩余时间: " + (int) (noLossTime - ship.getTimeDeployedForCRReduction()) + " sec", false);
                list.add(itemData);
            } else {
                CRStatusItemData itemData = new CRStatusItemData(statusKeys[6], icon, "作战压力",
                        "战备值衰减", true);
                list.add(itemData);
            }
        }
...
//	}
    ...
    public void applyCRToStats(float cr, MutableShipStatsAPI stats, ShipAPI.HullSize hullSize) {
        ...
        boolean fighter = hullSize == ShipAPI.HullSize.FIGHTER;
...
//		}
    ...
    //Purely Visual
    public float getSeriousMalfunctionThreshold(MutableShipStatsAPI stats) {
        float mult = 1f;
        if (stats != null) mult *= stats.getDynamic().getStat(Stats.CR_MALFUNCION_RANGE).getModifiedValue();
        return SERIOUS_MALFUNCTION_START * mult - 0.001f;
    }
    ...
    public CREffectDescriptionForTooltip getCREffectDescription(float cr, String shipOrWing, FleetMemberAPI member) {
        ...
        String str;
        if (cr < noDeploy) {
            str = String.format("%s 没有做好战斗准备，无法投入战斗。", shipOrWing);
        } else if (cr < getCriticalMalfunctionThreshold(member.getStats())) {
            str = String.format("%s 的作战性能下降，且在部署后有可能出现永久性和破坏性故障。", shipOrWing);
        } else if (cr < getMalfunctionThreshold(member.getStats())) {
            str = String.format("%s 的作战性能下降，在战斗中可能会出现武器和发动机故障。", shipOrWing);
        } else if (cr < getDegradeThreshold(member.getStats()) && hasNegative) {
            str = String.format("%s 在战斗中的性能下降。", shipOrWing);
        } else if (cr < getImproveThreshold(member.getStats()) || !hasPositive) {
            str = String.format("%s 能发挥出标准的战斗性能。", shipOrWing);
        } else {
            str = String.format("%s 的战斗能力提升了。", shipOrWing);
        }
        ...
        if (member.isFighterWing()) {
            ...
            if (canReplaceFighters) {
                ...
                str += " " + numStr + " 战机底盘已准备就绪，可随时补充战斗损失。";
                result.getHighlights().add(numStr);

            } else {
                details.add(new CREffectDetail("替换底盘", "无", CREffectDetailType.PENALTY));
                str += " " + "由于舰船没有配备适当设施，战机的损失无法弥补 (即缺乏 飞行甲板).";
            }
        }
    ...
    private String getMalfunctionString(MutableShipStatsAPI stats, float cr) {
        ...
        if (cr < getCriticalMalfunctionThreshold(stats)) {
            malfStr = "致命";
        } else if (cr < getSeriousMalfunctionThreshold(stats)) {
            malfStr = "严重";
    ...
    public List<CREffectDetail> getCREffectDetails(float cr, FleetMemberAPI member) {
        ...
        result.add(new CREffectDetail("航速 & 机动性", speedStr, getTypeFor(speed, false)));
        result.add(new CREffectDetail("受到伤害", damageStr, getTypeFor(damage, true)));
        result.add(new CREffectDetail("造成伤害", rofStr, getTypeFor(damageDealt, false)));

        if (member.getNumFlightDecks() > 0) {
            result.add(new CREffectDetail("战机整备时间", refitStr, getTypeFor(refit, true)));
        }

        result.add(new CREffectDetail("自动开火精度", accString, accType));
        ...
        if (hasMissiles) {
            ...
            if (missileFraction < 1f) {
                result.add(new CREffectDetail("导弹备弹", missileStr, missileFraction < 1 ? CREffectDetailType.PENALTY : CREffectDetailType.NEUTRAL));
            }
        }
