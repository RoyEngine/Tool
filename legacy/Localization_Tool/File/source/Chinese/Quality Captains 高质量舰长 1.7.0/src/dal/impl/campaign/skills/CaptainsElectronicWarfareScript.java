...
public class CaptainsElectronicWarfareScript extends BaseEveryFrameCombatPlugin {
	...
	public void init(CombatEngineAPI engine) {
		...
		if (APPLY_TO_TURRETS) {
			if (APPLIES_TO_RECOIL) { penTypes.add("后坐力"); }
			if (APPLIES_TO_RANGE) { penTypes.add("射程"); }
			if (APPLIES_TO_AUTOAIM) { penTypes.add("自动开火精度"); }
			if (APPLIES_TO_TURRET_TURNRATE) { penTypes.add("炮塔转速"); }
		}

		if (APPLY_TO_MISSILES && APPLIES_TO_MISSILE_TURNRATE) { penTypes.add("导弹机动性"); }
	...
	public void advance(float amount, List<InputEventAPI> events) {
		...
		String dataE = spaceStr + "-" + (int) enemyPenalty + "% 敌军" + penaltyStr;
		...
		if (playerPenalty > 0) {
			...
			String sMax = "";
			if (eMax <= playerPenalty) sMax = " (上限)";
			String title = "敌军电子对抗强度: " + (int) eTotal + "%";
			String data = "" + (int)playerPenalty + "% 我军受到影响" + sMax;
			if (eMax <= 0) {
				dataA = "完全抵消";
			}
		...
		if (enemyPenalty > 0) {
			...
			String sMax = "";
			if (pMax <= enemyPenalty) sMax = " (上限)";
			String title = "友军电子对抗强度:" + " " + (int) pTotal + "%";
			String data = "" + (int)enemyPenalty + "% 敌军受到影响" + sMax;
			if (pMax <= 0) {
				data = "完全抵消";
			}
		...
		if (playerPenalty > 0 && eMax > 0 && engine.getPlayerShip() != null) {
			...
			//eccm = -eccm;
			if (eccm != 0) {
				engine.maintainStatusForPlayerShip(KEY_STATUS2, icon, "我方电子反制套件抵消 ", "" + eccm + "% 敌方电子对抗强度", false);
			}
		}
