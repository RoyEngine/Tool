...
public class CaptainsBabylon {
	...
	public static final String skillName = "巴比伦人";
	public static final String skillDesc = "我们曾经团结一体，众志成城。而如今我们支离破碎，汝等之罪行永世铭记。";
	public static final String skillAuthor = "不明";
	...
	public static class Level1 implements ShipSkillEffect, AfterShipCreationSkillEffect {
		...
		public String getEffectDescription(float level) {
			if (enabled) {
				if (USE_UNNERF && PhaseCloakStats.FLUX_LEVEL_AFFECTS_SPEED) {
					return "相位时不再因不稳定遭受航速降低";
				}
				return "相位舰获得 " + Math.round(SPEED_BONUS*100) + "% 航速增益";
			}
			return "";
		}
		...
		for (SkillLevelAPI skill : Global.getSector().getPlayerStats().getSkillsCopy()) {
			if (skill.getSkill().getId().equals(skillID)) {
				if (skill.getLevel()==0) {
					...
					skill.getSkill().setDescription("击败异界的敌人。");
