...
public class CaptainsPhaseCorps {
	...
	public static class Level0 implements DescriptionSkillEffect {
		public String getString() {
			return "\n*相位力场 舰船插件将根据相位舰船的传感器探测强度为舰队提供隐蔽性增益，本技能带来的传感器探测强度增益也会计入其中。";
		}
	...
	public static class Level1 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			return "-" + Math.round(FLUX_UPKEEP_REDUCTION) + "% 激活相位隐形所产生的幅能";
		}
	...
	public static class Level2 implements ShipSkillEffect {
		...
		public String getEffectDescription(float level) {
			return "-" + Math.round(PHASE_COOLDOWN_REDUCTION) + "% 相位隐形的冷却时间";
		}
	...
	public static class Level3 extends BaseSkillEffectDescription implements ShipSkillEffect {
		...
		public void createCustomDescription(MutableCharacterStatsAPI stats, SkillSpecAPI skill, 
			...
			if (USE_PPT_BONUS) {
				info.addPara("+"+ (int) PEAK_TIME_BONUS + " 秒作战相位舰船峰值时间", hc, 0f);
			}
			if (USE_SPEED_BONUS) {
				info.addPara("+"+ (int) PHASE_SPEED_BONUS + "% 相位隐形下的最高航速与加速度", hc, 0f);
			}
			info.addPara("+"+(int)PHASE_SHIP_SENSOR_BONUS_PERCENT+"% 战斗相位舰船* 的传感器探测强度*", hc, 0f);
			...
			info.addSpacer(5f);
			if (USE_PPT_BONUS) {
				info.addPara("+%s 秒峰值时间 for combat phase ships", 0f, hc, "" + (int) PEAK_TIME_BONUS);
			}
			if (USE_SPEED_BONUS) {
